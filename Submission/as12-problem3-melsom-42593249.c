...
Description: This program solves the wave function of a particle inside of a
harmonic potential with a central barrier. 

Inputs: The user is required to enter the Barrier height, the lower and upper
...
time step iteration. The initial conditions call upon a function to populate
...
file (which is named in the variable declarations) which contains the columns
time position, norm, x_expect, x_stand_dev, prob_neg
...
int main () {
  ...
  double norm; // Wave function values
  double x_expect;
  double x_stand_dev;
  double psi_mod_squ;
  double prob_neg;
  ...
  char fname[]= "temp150.dat"; // For file output.
  FILE *dat;
  
  printf("This program will solve the wave function of a particle in a harmonic"
         " potential with an potential barrier.\n");
  ...
  // Checks that all in-putted values are OK.
  if ((scan_temp!= 2)
   || (b_nought< 0.0)
   || (x_final< x_initial)
   || (J< 0)
   ...
   || (t_final< t_out)) {
	...
	// Calculate spatial step size, temporal final step, and time output steps.
  ...
  for (time_count= 0; time_count< N; time_count++) {
    ...
    if (t_position >= t_output) {
      ...
      prob_neg= 0.0;
      for (count= 0; x_position[count]< 0.0; count++) {
        prob_neg += delta_x* (pow(creal(psi_new[count]), 2.0)+ 
      ...
      printf("%lf\t%lf\t%lf\t%lf\t%lf\n", 
            t_position, norm, x_expect, x_stand_dev, prob_neg);
